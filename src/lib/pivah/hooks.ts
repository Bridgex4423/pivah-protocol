...
/** Same reasoning and retry/split strategy as fetchSoldChunk above, adapted
 *  for a pool's Buy/Sell events instead of the Marketplace's Sold event. */
async function fetchTradeChunk(
  ...
  try {
    return (await publicClient!.getContractEvents({
      ...
      fromBlock: from,
      toBlock: to,
    })) as unknown as { args: { total?: bigint } }[];
  } catch (err) {
    if (attempt < 2) {
      await new Promise((r) => setTimeout(r, 300 * 2 ** attempt));
      return fetchTradeChunk(publicClient, pool, eventName, from, to, attempt + 1);
    }
    const span = to - from;
    if (span <= 2_000n) throw err;
    const mid = from + span / 2n;
    const [first, second] = await Promise.all([
      fetchTradeChunk(publicClient, pool, eventName, from, mid),
      fetchTradeChunk(publicClient, pool, eventName, mid + 1n, to),
    ]);
    return [...first, ...second];
  }
}

/** Real 24h DEX volume, summed from every pool's own Buy/Sell events —
 *  same honest, on-chain-only approach as the price chart and marketplace
 *  sales stats. Base produces a block roughly every 2 seconds, so 24h is
 *  ~43,200 blocks; the window here is rounded up with margin. */
export function useDex24hVolume() {
  const { pools } = usePools();
  const publicClient = usePublicClient();
  const chainId = useChainId();
  const poolAddresses = pools.map((p) => p.address).join(",");

  return useQuery({
    queryKey: ["dex-24h-volume", poolAddresses, chainId],
    enabled: Boolean(publicClient) && pools.length > 0,
    refetchInterval: 3 * 60_000,
    staleTime: 60_000,
    retry: 1,
    queryFn: async (): Promise<bigint> => {
      if (!publicClient || pools.length === 0) return 0n;

      const latest = await publicClient.getBlockNumber();
      const LOOKBACK = 45_000n; // ~24h on Base with margin
      ...
      let cursor = latest;
      while (cursor > from) {
        const chunkFrom = cursor - CHUNK > from ? cursor - CHUNK : from;
        ranges.push({ from: chunkFrom, to: cursor });
        cursor = chunkFrom;
      }

      // Buy and Sell chunks all fire together now instead of Buy-then-Sell
      // in sequence — halves the wall-clock time for the common case.
      // A hard timeout guarantees the UI never sits on "…" forever if a
      // request genuinely hangs — it surfaces a real error instead, which
      // reads as "couldn't load" rather than a silent stall.
      const timeout = new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error("24h volume scan timed out")), 20_000),
      );
      const scan = Promise.all(
        pools.flatMap((pool) =>
          (["Buy", "Sell"] as const).flatMap((eventName) =>
            ranges.map((r) => fetchTradeChunk(publicClient, pool.address, eventName, r.from, r.to)),
          ),
        ),
      );
      const allResults = await Promise.race([scan, timeout]);

      let volume = 0n;
      for (const chunk of allResults) {
        for (const log of chunk) {
          if (log.args.total !== undefined) volume += log.args.total;
        }
      }
      return volume;
    },
