...
function DexPage() {
  ...
  // Pools with nothing in them — no NFTs, no WETH — usually mean a deposit
  // transaction failed partway through (the pool itself still deploys as
  // its own transaction; it's the liquidity step right after that can
  // fail separately). They're not tradeable, so they're worth keeping out
  // of the main browsing list — but a pool contract, once created, is
  // permanent on-chain and can't actually be removed, so this only hides
  // it from the list; it stays fully reachable directly if you ever want
  // to add liquidity to it and bring it to life.
  ...
  const filteredPools = useMemo(() => {
    if (!q) return browsablePools;
    return browsablePools.filter(
      (p) =>
