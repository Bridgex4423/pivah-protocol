...
function StakePage() {
  ...
  async function stakeSelected() {
    ...
    if (verified.length === 1) {
      ...
      // Staking many NFTs in one transaction can push gas high enough to
      // fail or time out, the same way a large mint or marketplace listing
      // batch can — split into safe-sized batches, same threshold already
      // validated for those.
      let done = 0;
      ...
      while (done < verified.length) {
        ...
        receipt = await send(
          totalBatches > 1
            ? `Stake batch ${batchNumber}/${totalBatches}`
            : `Stake ${verified.length} NFTs`,
          {
            address: vaultAddress,
            abi: stakingVaultAbi,
            functionName: "stakeBatch",
            args: [collection as Address, chunk],
          },
        );
        done += chunk.length;
      }
  ...
  async function unstakeAll() {
    ...
    // Same batching treatment as staking itself — unstaking many NFTs in
    // one transaction carries the same risk of failing or timing out.
    const allIds = vault.myStakes.map((s) => s.stakeId);
    let done = 0;
    let receipt: Awaited<ReturnType<typeof send>> | undefined;
    while (done < allIds.length) {
      const chunk = allIds.slice(done, done + MINT_BATCH_SIZE);
      const batchNumber = Math.floor(done / MINT_BATCH_SIZE) + 1;
      const totalBatches = Math.ceil(allIds.length / MINT_BATCH_SIZE);
      receipt = await send(
        totalBatches > 1
          ? `Unstake batch ${batchNumber}/${totalBatches}`
          : `Unstake ${allIds.length} NFTs`,
        {
          address: addresses.stakingVault as Address,
          abi: stakingVaultAbi,
