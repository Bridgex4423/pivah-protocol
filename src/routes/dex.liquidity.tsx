...
function LiquidityCard({ pool, onDone }: { pool: PoolSummary; onDone: () => void }) {
  ...
  async function addLiquidity() {
    ...
    // batch can — split into safe-sized batches. WETH only goes with the
    // first batch: the contract sets the pool's price from that first
    // deposit, and every batch after it correctly mints shares for
    // NFT-only additions once a price is set.
    let done = 0;
    let receipt: Awaited<ReturnType<typeof send>> | undefined;
    if (verified.length === 0) {
      receipt = await send("Add liquidity", {
        address: pool.address,
        abi: poolAbi,
        functionName: "addLiquidity",
        args: [[], wethAmount, 0n, address],
      });
    } else {
      while (done < verified.length) {
        ...
        receipt = await send(
          totalBatches > 1 ? `Deposit batch ${batchNumber}/${totalBatches}` : "Add liquidity",
          {
            address: pool.address,
            abi: poolAbi,
            functionName: "addLiquidity",
