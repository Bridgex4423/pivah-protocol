...
function PoolsPage() {
  ...
  // Same reasoning as the Swap page's pool picker: a pool with nothing in
  // it isn't tradeable, and usually means a deposit failed partway
  // through rather than that anyone intended an empty pool. The contract
  // itself is still permanent on-chain either way — this only keeps it
  // out of this browsing list.
  const browsablePools = useMemo(
    () => pools.filter((p) => p.inventory > 0n || p.quoteReserves > 0n),
    [pools],
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard label="Pools" value={String(browsablePools.length)} />
        <StatCard
...
function CreatePool({
  ...
  async function create() {
    ...
    // Depositing many NFTs in one transaction can push gas high enough to
    // fail or time out — same reasoning, same batch size already proven
    // safe for minting, marketplace listing, and adding liquidity to an
    // existing pool. WETH only goes with the first batch: that call sets
    // the pool's starting price, and every batch after it correctly adds
    // NFT-only liquidity once a price is set.
    let done = 0;
    while (done < verified.length) {
      const chunk = verified.slice(done, done + MINT_BATCH_SIZE);
      const batchNumber = Math.floor(done / MINT_BATCH_SIZE) + 1;
      const totalBatches = Math.ceil(verified.length / MINT_BATCH_SIZE);
      if (totalBatches > 1) setStep(`Depositing batch ${batchNumber}/${totalBatches}`);
      await send(totalBatches > 1 ? `Seed pool ${batchNumber}/${totalBatches}` : "Seed pool", {
        address: pool,
        abi: poolAbi,
        functionName: "addLiquidity",
        args: [chunk, done === 0 ? wethAmount : 0n, 0n, address],
      });
      done += chunk.length;
    }
